...
Element CreateFolderDialog::render() {
    ...
    dialog_content.push_back(
        hbox({
            text(" "),
            text(icons::FOLDER) | color(colors.keyword) | bold,
    ...
    std::string display_dir = current_directory_.length() > 40 ? 
        "..." + current_directory_.substr(current_directory_.length() - 37) : current_directory_;
    dialog_content.push_back(
        hbox({
            text("  "),
            text(icons::LOCATION) | color(colors.comment),
            text(" Location: "),
    ...
    dialog_content.push_back(
        hbox({
            text("  "),
            text("Folder name: ") | color(colors.foreground),
            text(" "),
            text(folder_input_display) | bold | color(colors.background) | bgcolor(colors.selection)
        })
    );
